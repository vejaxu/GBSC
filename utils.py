"""Shared data, metric, formatting, and plotting utilities for GBSC."""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/gbsc-matplotlib")
...
def load_dataset(path: Path | str, name: str | None = None) -> Dataset:
    ...
    n_samples, n_features = map(int, features.shape)
    return Dataset(
...
def plot_clustering(
    ...
    points = np.asarray(projected_features)[indices]
    aligned = align_labels_hungarian(true_labels, predicted_labels)
    _scatter(points, aligned[indices], output_dir / "clustering_result.jpg")
    _scatter(points, np.asarray(true_labels)[indices], output_dir / "true_labels.jpg")
