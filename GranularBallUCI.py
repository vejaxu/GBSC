...
def gb_plot(gbs, key, noise):
    ...
    # plt.show()
    plt.savefig(f"fig/gb_plot_{key}.png")
...
def getGranularBall(key):
    ...
    # with h5py.File(f"../data/noise_data/{key}.mat", 'r') as file:
    #     # print("Keys in the file:", list(file.keys()))
    #     data = file['data_with_noise'][()]
    #     target = file['class'][()]
    #     data = data.T
    #     target = target.T

    if key == "skin":
        with h5py.File(f"../data/{key}.mat", 'r') as file:
            data = file['fea'][()]
            target = file['gt'][()]
            data = data.T
            target = target.T
    else:
        dict_mat = loadmat(f"../data/{key}.mat")
        if key == "pendigits":
            data = dict_mat['X']
            target = dict_mat['gtlabels'].T
        elif key == 'landsat' or key == 'cure-t2-4k' or key == 'waveform3':
            data = dict_mat['data']
            target = dict_mat['label']
        elif key == 'letters':
            data = dict_mat['fea']
            target = dict_mat['gt']
        else:
            data = dict_mat['data']
            target = dict_mat['class']
